...
// HELM-GWLOG-IMPORT-agentgateway-1: RED-before-GREEN row test.
//
...
//   - agentgateway.golden.jsonl imports to a generic log that seals into a
//     bundle that verifies VALID (GOLDEN);
//   - agentgateway.tampered.jsonl — the SAME log with ONE byte flipped inside
//     a request_digest — cannot become a bundle: the importer cross-checks a
//     body-bearing line's digest against sha256(body) and refuses; and the
//     tamper the log flip represents in the sealed record (the flipped digest
...
//     (RUNNER-REFUSAL).
// The run order in CI is RED (before the importers exist) then GREEN.
import { test } from "node:test";
import assert from "node:assert/strict";
import { readFileSync } from "node:fs";
import { join, dirname } from "node:path";
import { fileURLToPath } from "node:url";
import { importAgentGatewayLog as importLog } from "./agentgateway.mjs";
import { verifyBundle } from "../../ui/lib/verify-bundle.mjs";
import { hashBody } from "./_map.mjs";

const HERE = dirname(fileURLToPath(import.meta.url));
const GOLDEN = readFileSync(join(HERE, "..", "..", "fixtures", "gwlog", "agentgateway.golden.jsonl"), "utf8");
const TAMPERED = readFileSync(join(HERE, "..", "..", "fixtures", "gwlog", "agentgateway.tampered.jsonl"), "utf8");
const GOLDEN_ID = "bundle-gwlog-agentgateway-1";
const TAMPER_ID = "bundle-gwlog-agentgateway-tampered-1";
...
test("BODY-DROP: a raw request body is hashed and never reaches the bundle", () => {
  const body = "PROMPT-AND-COMPLETION-SECRET-PAYLOAD-body-must-not-reach-the-bundle";
  const jsonlText = JSON.stringify({
    ts: "2026-09-09T12:00:00.000Z",
    "http.host": "tools.example.com",
    "http.method": "POST",
    "http.path": "/v1/tools/echo",
    "request.body": body,
  ...
  // cross-check the digest is the recorded body hash:
