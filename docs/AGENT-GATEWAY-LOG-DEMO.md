...
- `scripts/gwlog-import/openclaw.mjs`: the OpenClaw map. The OpenClaw Gateway writes a rolling JSON-lines **file log** (`/tmp/openclaw/openclaw-YYYY-MM-DD.log`, "one JSON object per line" per `docs.openclaw.ai/logging/` and `docs.openclaw.ai/gateway/logging/`): the map reads the documented `time`/`level`/`message` envelope keys and the documented machine-filterable top-level fields `agent_id` (→ `actor_id`), `session_id` (→ `run_id`), `channel` (→ scope), plus the preserved numbered tslog argument keys for a bound tool name; the documented always-info `[model-fetch]` hook records model calls with the `url` host as `target_host`. The file log carries no digest fields and never carries payload bodies (tool arguments/results are never stored), so `request_digest`/`response_digest` and `request.body`/`response.body` are the off-box export sidecar (same hash-drop/`digest_mismatch`/`no_digest` policy as above); tool-name/tool-profile/tool-target and tool-version are not documented per-line, so `actor_version` is `"unknown"` and scope comes only from the sidecar `tool_profile` or the documented `channel` field.
- `scripts/gwlog-import/agent365.mjs`: the Microsoft Agent 365 / Purview audit export map. Purview audit records for Copilot and AI applications (see `learn.microsoft.com/en-us/purview/audit-copilot` for `AgentId`/`AgentName`/`AgentVersion`, `Operation` — `CopilotInteraction`/`ConnectedAIAppInteraction`/`AIAppInteraction` —, `RecordType`/`Workload`/`AppHost`, and the `Messages` prompt-response pair, plus the Office 365 Management Activity API common schema envelope at `learn.microsoft.com/en-us/office/office-365-management-api/office-365-management-activity-api-schema/` for `Id`/`CreationTime`/`UserId`) map `AuditData.AgentId` (the Entra Agent ID the export carries, pass-through) to `actor_id`, `AuditData.AgentVersion` to `actor_version`, `Operation` to `action`, `AppHost`/`Workload` to `target_host`. Purview records carry no digests; the doc'd `AuditData.Messages` object is hashed (sha256) and DROPPED, and a record carrying neither it nor an enrichment digest is refused with `no_digest`.


- `scripts/gwlog-import/cfportal.mjs`: the Cloudflare MCP Server Portal (Logpush) map. Maps the `mcp_portal_logs` Logpush dataset (`developers.cloudflare.com/logs/logpush/logpush-job/datasets/account/mcp_portal_logs/`: `Datetime`, `Method`/`ToolCallName`/`PromptGetName`/`ResourceReadURI`, `UserID`/`UserEmail`, `SessionID`, `PortalID`/`ServerID`/`ServerURL`, `Success`/`Error`) to the generic shape; governance context: `developers.cloudflare.com/agents/model-context-protocol/protocol/governance/`. The documented dataset carries no body, digest, or DLP field, so the map reads operator-added enrichment fields (`request_body`/`response_body`, `request_digest`/`response_digest`, `dlp_label`) and marks undocumented values `"unknown"` — per that dataset's own field list, verified 2026-09-09.
