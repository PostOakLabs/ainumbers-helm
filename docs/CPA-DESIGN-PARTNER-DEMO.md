...
- Node.js ≥ 22.5 (`package.json` `engines.node`) — no `npm install` needed for
  this walkthrough; it only imports modules already in the repo.
...
const keys = loadOrCreateKeys(); // demo keys — see loadOrCreateKeys() for where these live
const sealed = [];
...
- Real data never belongs in this repo, in `helm/fixtures/`, or in any commit
  ...
  ([CONTRACT §0 Zero-PII posture](../../repo/CONTRACT.md), extended by analogy
  per `CPA-RAILS-BUILD-SPEC.md` §5 item 1).
