/* Vendored: @noble/curves (secp256k1 ECDSA verify path only) + @noble/hashes (sha3/keccak,
   sha2, hmac) v2.2.0 (MIT, (c) Paul Miller paulmillr.com). Self-contained ESM bundle exporting
   { secp256k1, keccak_256, sha3_256 }. DO NOT hand-edit -- rebuild from pinned source (see
   FINP2P-VERIFY-BUILD-1 build notes / scratchpad/noble/build.mjs).
   Source: https://github.com/paulmillr/noble-curves (secp256k1.js + abstract/{utils,modular,curve,
   weierstrass}.js) and https://github.com/paulmillr/noble-hashes (utils.js, _u64.js, _md.js, sha2.js,
   hmac.js, sha3.js), pinned to npm tag v2.2.0 -- same pin already vendored in
   chaingraph/kernels/_noble-bn254.bundle.mjs.
   Function bodies are byte-identical to the pinned npm dist output; only import/export boilerplate
   was stripped and a handful of same-name top-level bindings were disambiguated when flattening
   multiple source files into one scope (documented inline below). Schnorr (BIP340), FROST, and
   RFC 9380 hash-to-curve are DELETED from the vendored secp256k1.js -- this node only needs ECDSA
   verify(), and dropping them avoids vendoring frost.js / hash-to-curve.js entirely. Signing
   (sign()) rides along unused but harmless, exactly as the BN254 bundle's precedent for unused
   exports -- this node never calls it.
   License: MIT, (c) Paul Miller paulmillr.com. Full text: https://github.com/paulmillr/noble-curves/blob/main/LICENSE
   and https://github.com/paulmillr/noble-hashes/blob/main/LICENSE (identical MIT text, same author). */
...
export { secp256k1, keccak_256, sha3_256 };
