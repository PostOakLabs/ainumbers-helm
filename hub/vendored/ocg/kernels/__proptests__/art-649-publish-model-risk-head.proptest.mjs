// art-649-publish-model-risk-head — class-B property-test floor.
// kernel_digest_at_authoring: sha256:3f1c6d1e5f697c7965be668eb206a5c9d533b097a4d381c90c48aefaafdf0f91
// spec: MRM-LINEAGE-BUILD-SPEC.md §4
...
// verify() bridge was measured to diverge from the worker during authoring). This floor's job is
// compute()'s own synchronous contract per FV-PBT-FLOOR-BUILD-SPEC.md §3.
//
// ZERO external dependencies — Node built-ins plus the in-repo _pbt-common.mjs helpers only.
//
// Run: node chaingraph/kernels/__proptests__/art-649-publish-model-risk-head.proptest.mjs

import { compute } from '../art-649-publish-model-risk-head.kernel.mjs';
import { summarize, mulberry32, pick, findShapeViolations, FIXTURES_DIR } from './_pbt-common.mjs';
import { readFileSync } from 'node:fs';
import { join } from 'node:path';

const KERNEL_ID = 'art-649-publish-model-risk-head';
const rand = mulberry32(0x649D2);
const TRIALS = 8000;
...
// basis — same precedent as art-55-trade-document-provenance-verifier's merkle_root.
...
function randValidHead(rng, seq) {
  ...
  return {
    head_version: '1',
    stream: 'model-risk-head:MODEL-' + Math.floor(rng() * 1000),
    signer,
    seq,
...
function checkP1_validGenesisAlwaysPasses() {
  ...
  for (let i = 0; i < TRIALS; i++) {
    ...
    if (!r.compliance_flags.includes('MRM_HEAD_STRUCTURE_VALID') || !r.compliance_flags.includes('MRM_HEAD_GENESIS')) violations++;
  }
...
function checkP2_validChainedAlwaysPasses() {
  ...
  for (let i = 0; i < TRIALS; i++) {
    ...
    if (r.output_payload.structural_error !== null) violations++;
    if (r.output_payload.is_genesis) violations++;
    if (!r.compliance_flags.includes('MRM_HEAD_CHAINED')) violations++;
    if (head.rotates_to && !r.compliance_flags.includes('MRM_HEAD_ROTATION_ANNOUNCED')) violations++;
  }
...
function checkP3_mutationAlwaysDetected() {
  ...
  for (let i = 0; i < TRIALS; i++) {
    ...
    if (r.output_payload.structural_error === null) violations++;
    if (!r.compliance_flags.includes('MRM_HEAD_STRUCTURAL_ERROR')) violations++;
  }
...
const FORCED_CASES = [
  ...
  [{ head: G, signature_verification: OK_CLAIM }, 'well-formed genesis with claim — MRM_HEAD_GENESIS'],
  [{ head: C, signature_verification: OK_CLAIM }, 'well-formed chained head with claim, no prior_head — MRM_HEAD_CHAINED'],
  ...
  [{ head: C, signature_verification: OK_CLAIM, prior_head: randValidHead(mulberry32(3), 4), chain_verification: OK_CLAIM }, 'chained head with prior_head and chain_verification — MRM_HEAD_CHAIN_CLAIMED_VALID'],
  [{ head: { ...C, rotates_to: 'did:key:zROTATED' }, signature_verification: OK_CLAIM }, 'rotation announced — MRM_HEAD_ROTATION_ANNOUNCED'],
];
