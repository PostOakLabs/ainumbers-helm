// kernel_digest_at_authoring: sha256:ebdcc54a5a9f44bae3307929b2c226e91666a2929a99fb42e15eb5f0df79c7d1
//
// FV-PROPFLOOR-SHARD-B23-1 — property-test floor for art-432-call-report-rc-balance-sheet.
...
// declared rounding tolerance) — ULP-boundary forcing is MANDATORY per FV-PBT-FLOOR-BUILD-
// SPEC.md §3. Zero external dependencies. This file is READ-ONLY with respect to the kernel
// it imports.
//
// human_sign_off: PENDING (this row does not sign — manifest-level signature per spec §4)
//
// Run: node chaingraph/kernels/__proptests__/art-432-call-report-rc-balance-sheet.proptest.mjs

import { compute } from '../art-432-call-report-rc-balance-sheet.kernel.mjs';
...
function runFixtureOracle() {
  const fixturesPath = path.join(__dirname, '..', 'fixtures', 'art-432-call-report-rc-balance-sheet.fixtures.json');
...
const rand = mulberry32(0x432C3);
function randRange(rng, lo, hi) { return lo + rng() * (hi - lo); }
const TRIALS = 10000;
function r2(v) { return Math.round(v * 100) / 100; }

function mkPP(rng, { balance = false } = {}) {
  const pp = { entity_id: 'BANK-1', reporting_period: '2026Q2' };
  ...
  if (balance) {
    // force the identity to hold exactly by setting total_deposits_usd as the plug.
...
const ULP_BOUNDARY_CASES = [
  ...
  [{ cash_and_due_from_usd: 1.005, total_deposits_usd: 1 }, 'asset item at the r2 half-cent rounding boundary (1.005) — Math.round(v*100)/100 banker rounding behavior must be deterministic'],
...
function checkP4_forced() {
  ...
  for (const [partial, label] of ULP_BOUNDARY_CASES) {
    const pp = { entity_id: 'BANK-1', reporting_period: '2026Q2', ...partial };
    const r = compute(pp);
    const op = r.output_payload;
