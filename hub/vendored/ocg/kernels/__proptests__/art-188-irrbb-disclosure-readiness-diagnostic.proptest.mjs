// art-188-irrbb-disclosure-readiness-diagnostic property-test floor (FV-PROPFLOOR-SHARD-A-BOOLDIM-1).
// kernel_digest_at_authoring: sha256:9339e768762bca73b88956e51e3ffc9500c8f2a6a54cba095f989992be265292
// human_sign_off: PENDING
//
// Class-A floor per FV-PBT-FLOOR-BUILD-SPEC.md §3 -- cheap invariant subset over the DECLARED domain,
// not a totality proof. Domain is 5 independent booleans read via `=== true` -- 2^5 = 32 states, small
...
//
// Run: node chaingraph/kernels/__proptests__/art-188-irrbb-disclosure-readiness-diagnostic.proptest.mjs

import { compute } from '../art-188-irrbb-disclosure-readiness-diagnostic.kernel.mjs';
...
function runFixtureOracle() {
  const fixturesPath = path.join(__dirname, '..', 'fixtures', 'art-188-irrbb-disclosure-readiness-diagnostic.fixtures.json');
...
function negativeControl() {
  const good = { eve_shock_calculated: true, sot_eve_nii_evaluated: true, standardised_approach_mapped: true, csrbb_scope_assessed: true, pillar3_irrbb1_ready: true };
...
const DIM_KEYS = ['eve_shock_calculated', 'sot_eve_nii_evaluated', 'standardised_approach_mapped', 'csrbb_scope_assessed', 'pillar3_irrbb1_ready'];
const GRADES = ['F', 'F', 'D', 'C', 'B', 'A']; // index 0..5 met -- note: only 6 entries, unlike the 6-dim siblings' 7-entry table

// P1: full 2^5 enumeration -- dimensions_met, readiness_score, grade, gaps all agree exactly with the mask.
function checkP1_fullEnumAgreement() {
  ...
  for (let mask = 0; mask < 32; mask++) {
    ...
    checked++;
    const expectedScore = Math.round((expectedMet / 5) * 100);
    ...
    if (output_payload.fully_ready !== (expectedMet === 5)) violations++;
    if (JSON.stringify(output_payload.gaps) !== JSON.stringify(expectedGaps)) violations++;
  }
  return { name: 'P1_full_32_state_enum_agreement', trials: checked, violations };
}
...
function checkP4_outputShapeInvariant() {
  let violations = 0, checked = 0;
  const inputs = [{}, { entity: {} }, { entity: { eve_shock_calculated: true } }, { entity: { sot_eve_nii_evaluated: true, pillar3_irrbb1_ready: true } }];
...
console.log(JSON.stringify({
  kernel_id: 'art-188-irrbb-disclosure-readiness-diagnostic',
