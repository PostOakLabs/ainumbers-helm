// art-176-ai-governance-readiness-diagnostic property-test floor (FV-PROPFLOOR-SHARD-A-BOOLDIM-1).
// kernel_digest_at_authoring: sha256:9eade7087913b370413de5a95c491f8c88aa907701afba40c373dc637beef62d
...
//
// Run: node chaingraph/kernels/__proptests__/art-176-ai-governance-readiness-diagnostic.proptest.mjs

import { compute } from '../art-176-ai-governance-readiness-diagnostic.kernel.mjs';
...
function runFixtureOracle() {
  const fixturesPath = path.join(__dirname, '..', 'fixtures', 'art-176-ai-governance-readiness-diagnostic.fixtures.json');
...
function negativeControl() {
  const good = { aims_documented: true, impact_assessed: true, nist_rmf_mapped: true, gpai_obligations_checked: true, monitoring_plan_active: true, review_cycle_defined: true };
...
const DIM_KEYS = ['aims_documented', 'impact_assessed', 'nist_rmf_mapped', 'gpai_obligations_checked', 'monitoring_plan_active', 'review_cycle_defined'];
const GRADES = ['F', 'F', 'E', 'D', 'C', 'B', 'A'];

// P1: full 2^6 enumeration -- dimensions_met, readiness_score, grade, gaps, frameworks_addressed all
// agree exactly with the mask (frameworks_addressed derives from specific individual dims, not the count).
function checkP1_fullEnumAgreement() {
  ...
  for (let mask = 0; mask < 64; mask++) {
    ...
    const expectedFrameworks = {
      iso_42001: !!entity.aims_documented && !!entity.impact_assessed,
      nist_rmf: !!entity.nist_rmf_mapped,
      eu_ai_act: !!entity.gpai_obligations_checked,
    };
    if (JSON.stringify(output_payload.frameworks_addressed) !== JSON.stringify(expectedFrameworks)) violations++;
  }
...
function checkP3_strictBooleanRead() {
  ...
  for (const v of NEAR_TRUE) {
    ...
    if (output_payload.frameworks_addressed.iso_42001 || output_payload.frameworks_addressed.nist_rmf || output_payload.frameworks_addressed.eu_ai_act) violations++;
  }
...
function checkP4_outputShapeInvariant() {
  let violations = 0, checked = 0;
  const inputs = [{}, { entity: {} }, { entity: { aims_documented: true } }, { entity: { nist_rmf_mapped: true, review_cycle_defined: true } }];
  for (const pp of inputs) {
    ...
    if (typeof output_payload.frameworks_addressed !== 'object' || output_payload.frameworks_addressed === null) violations++;
  }
...
console.log(JSON.stringify({
  kernel_id: 'art-176-ai-governance-readiness-diagnostic',
