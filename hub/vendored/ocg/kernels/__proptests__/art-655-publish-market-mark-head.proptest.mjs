// art-655-publish-market-mark-head — class-B property-test floor.
// kernel_digest_at_authoring: sha256:6c5a944ad3d3508d828bc34a320e09734f576f2f1b8522d1f9b16eac803bd568
// spec: DERIV-WORKFLOWS-BUILD-SPEC.md §4
...
// verify() bridge was measured to diverge from the worker during art-649's authoring). This
// floor's job is compute()'s own synchronous contract per FV-PBT-FLOOR-BUILD-SPEC.md §3. Same
// shape as art-649-publish-model-risk-head's own floor (the landed PROV-HEAD head-kernel
// pattern) — reused, not reimplemented, with the stream naming updated to this node's
// "mark:<venue>:<pair>" convention.
//
// ZERO external dependencies — Node built-ins plus the in-repo _pbt-common.mjs helpers only.
//
// Run: node chaingraph/kernels/__proptests__/art-655-publish-market-mark-head.proptest.mjs

import { compute } from '../art-655-publish-market-mark-head.kernel.mjs';
import { summarize, mulberry32, pick, findShapeViolations, FIXTURES_DIR } from './_pbt-common.mjs';
import { readFileSync } from 'node:fs';
import { join } from 'node:path';

const KERNEL_ID = 'art-655-publish-market-mark-head';
const rand = mulberry32(0x655D2);
const TRIALS = 8000;
...
// basis — same precedent as art-649's own floor.
...
const VENUES = ['kalshi', 'hyperliquid', 'polymarket', 'dydx', 'aave'];
const PAIRS = ['BTCPERP', 'ETHPERP', 'TRADEXYZ-SPX', 'SOL-USD', 'HIP3-OI'];
...
function randValidHead(rng, seq) {
  ...
  return {
    head_version: '1',
    stream: 'mark:' + pick(rng, VENUES) + ':' + pick(rng, PAIRS),
    signer,
    seq,
...
function checkP1_validGenesisAlwaysPasses() {
  ...
  for (let i = 0; i < TRIALS; i++) {
    ...
    if (!r.compliance_flags.includes('MMH_HEAD_STRUCTURE_VALID') || !r.compliance_flags.includes('MMH_HEAD_GENESIS')) violations++;
  }
...
function checkP2_validChainedAlwaysPasses() {
  ...
  for (let i = 0; i < TRIALS; i++) {
    ...
    if (r.output_payload.structural_error !== null) violations++;
    if (r.output_payload.is_genesis) violations++;
    if (!r.compliance_flags.includes('MMH_HEAD_CHAINED')) violations++;
    if (head.rotates_to && !r.compliance_flags.includes('MMH_HEAD_ROTATION_ANNOUNCED')) violations++;
  }
...
function checkP3_mutationAlwaysDetected() {
  ...
  for (let i = 0; i < TRIALS; i++) {
    ...
    if (r.output_payload.structural_error === null) violations++;
    if (!r.compliance_flags.includes('MMH_HEAD_STRUCTURAL_ERROR')) violations++;
  }
...
const FORCED_CASES = [
  ...
  [{ head: G, signature_verification: OK_CLAIM }, 'well-formed genesis with claim — MMH_HEAD_GENESIS'],
  [{ head: C, signature_verification: OK_CLAIM }, 'well-formed chained head with claim, no prior_head — MMH_HEAD_CHAINED'],
  ...
  [{ head: C, signature_verification: OK_CLAIM, prior_head: randValidHead(mulberry32(3), 4), chain_verification: OK_CLAIM }, 'chained head with prior_head and chain_verification — MMH_HEAD_CHAIN_CLAIMED_VALID'],
  [{ head: { ...C, rotates_to: 'did:key:zROTATED' }, signature_verification: OK_CLAIM }, 'rotation announced — MMH_HEAD_ROTATION_ANNOUNCED'],
];
